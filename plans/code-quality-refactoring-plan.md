...
The latest wiki-importance pass now normalizes `WikiStore` and `ChatInspectionService` boundaries to integer importance semantics, keeps DB storage compatible while enforcing positive integer inputs for create/edit/decay paths, and updates wiki/inspection/console coverage accordingly. Focused wiki store/server/inspection tests and the full `make precommit` gate remain green. Remaining persistence work in this item is now limited to documenting sync transaction boundaries explicitly and evaluating a future schema-level alignment step.
