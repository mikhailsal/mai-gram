...
The CLI history and wiki inspection paths now also delegate their DB/wiki-store reads through `src/mai_gram/core/chat_inspection_service.py`, so adapter-owned inspection logic is narrower and future Telegram-facing inspection features can reuse the same application service. The latest runtime-composition pass now routes both `src/mai_gram/main.py` and `src/mai_gram/console_runner.py` through `src/mai_gram/core/adapter_runtime.py` for OpenRouter provider, external MCP pool, and `BotHandler` construction, and the CLI prompt-preview path now receives the same external MCP pool wiring as Telegram startup. The follow-on adapter-coverage pass now adds direct tests for the display, inspection, import, and dispatch branches in `src/mai_gram/console_runner.py`, removes it from the coverage omit list, and keeps the full `make precommit` gate green with `console_runner.py` at `99%` direct coverage.
...
`src/mai_gram/config.py` now delegates file-backed behavior to `src/mai_gram/config_loaders.py`, which owns mtime-cached loading for `models.toml`, `bots.toml`, and prompt companion TOML files. The singleton `get_settings()` API remains intact for existing runtime call sites, but `src/mai_gram/main.py` now refreshes models config through the public `refresh_models_config()` facade instead of calling a private loader method directly, and its startup/shutdown flow now uses a typed `AppRuntime` container instead of module-level mutable globals. Replacing singleton-driven runtime composition inside downstream services with explicit loader and settings injection is a follow-on improvement beyond the scope of this plan.
...
`src/mai_gram/mcp_servers/messages_server.py` now keeps its MCP tool metadata in focused static specs and routes timerange requests through typed parsing and formatting helpers, so `MessagesMCPServer.list_tools` and `MessagesMCPServer._call_get_messages_by_timerange` no longer appear in the size audit while the direct messages-server and adjacent MCP bridge tests stay green. `src/mai_gram/mcp_servers/bridge.py` now acts as a thin public facade over `src/mai_gram/mcp_servers/bridge_support.py`, which holds the shared tool-loop orchestration, streaming accumulation, and callback dispatch helpers; the bridge file and `run_with_tools_stream()` have dropped out of the size audit while the direct bridge stream tests remain green. `src/mai_gram/mcp_servers/wiki_server.py` now exposes its MCP tool schemas through module-level static specs, so `WikiMCPServer.list_tools` has also dropped out of the size audit while the direct wiki-server and adjacent MCP registration tests stay green. `src/mai_gram/llm/openrouter.py` now delegates payload serialization plus streaming/response parsing to `src/mai_gram/llm/openrouter_support.py`, so the OpenRouter file and `OpenRouterProvider._stream_sse()` have dropped out of the size audit while direct provider streaming tests now also cover reasoning chunks, tool-call deltas, and BYOK cost handling. The latest external-server pass now parses JSON-RPC responses and external server config through typed helper models in `src/mai_gram/mcp_servers/external.py`, so the stdio bridge no longer decodes matching responses and config maps through raw ad hoc dict checks while the direct external-MCP tests and full repository gate remain green. The follow-on persistence-boundary pass now makes `src/mai_gram/memory/messages.py` the typed codec for stored roles and tool-call payloads, routes importer normalization and prompt-builder readback through that codec, and changes the assistant tool-call callback path to pass `ToolCall` objects internally instead of raw JSON strings before the DB boundary. The DB schema remains unchanged, but the main stored-message seam is now typed and covered by direct message-store, importer, prompt-builder, conversation-executor, MCP bridge, and import-service tests. Broader typed MCP boundary work across remaining provider-facing shapes is a follow-on improvement beyond the scope of this plan.
...
`src/mai_gram/core/importer.py` now uses focused helpers for entry validation, content/tool-call normalization, reasoning extraction, and persistence so `save_imported_messages` no longer appears in the size audit while the importer test suite and full live validation remain green. The follow-on persistence-boundary pass now also makes `src/mai_gram/core/prompt_builder.py` read-only with respect to wiki reconciliation and moves the pre-context `sync_from_disk()` step into `src/mai_gram/bot/assistant_turn_builder.py` and `src/mai_gram/core/prompt_preview_service.py`, so ordinary conversation/regenerate plus CLI prompt preview refresh wiki state through explicit workflow-owned sync points instead of hidden prompt-build side effects. Focused prompt-builder, assistant-turn-builder, and prompt-preview tests plus the full `make precommit` gate remain green. Broader persistence-normalization work, including enum-backed roles and structured persisted tool-call types, is a follow-on improvement beyond the scope of this plan.
The latest wiki-importance pass now normalizes `WikiStore` and `ChatInspectionService` boundaries to integer importance semantics, keeps DB storage compatible while enforcing positive integer inputs for create/edit/decay paths, and updates wiki/inspection/console coverage accordingly. Follow-on docs and service/store contracts now also document explicit sync transaction ownership (which workflows may trigger reconciliation and when callers should commit), so hidden write behavior is no longer implicit in this area. Focused wiki store/server/inspection tests and the full `make precommit` gate remain green. Remaining persistence work is limited to evaluating a future schema-level alignment step, which is a follow-on improvement.
