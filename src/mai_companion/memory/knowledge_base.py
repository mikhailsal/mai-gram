...
from mai_companion.db.models import KnowledgeEntry
...
class WikiStore:
    ...
    async def create_entry(
        ...
        *,
        ...
        existing = await self._get_entry(companion_id, safe_key)
        if existing is not None:
            raise ValueError(f"Wiki entry with key '{safe_key}' already exists")

        wiki_dir = self._wiki_dir(companion_id)
    ...
    async def edit_entry(
        ...
        *,
        ...
        safe_key = self._sanitize_key(key)
        entry = await self._get_entry(companion_id, safe_key)
        if entry is None:
            return None

        current_file = self._find_entry_file(companion_id, safe_key)
    ...
    async def read_entry(self, companion_id: str, key: str) -> str | None:
        """Read an entry by key, preferring file content over DB value."""
        safe_key = self._sanitize_key(key)
        file_path = self._find_entry_file(companion_id, safe_key)
    ...
    async def search_entries(
        ...
        *,
        ...
        result = await self._session.execute(
            ...
            .where(
                and_(
                    KnowledgeEntry.companion_id == companion_id,
                    or_(
    ...
    async def get_top_entries(
        ...
        *,
        ...
        result = await self._session.execute(
            select(KnowledgeEntry)
            .where(KnowledgeEntry.companion_id == companion_id)
            .order_by(desc(KnowledgeEntry.importance), desc(KnowledgeEntry.updated_at))
            .limit(limit)
        )
        return list(result.scalars().all())

    async def list_entries(self, companion_id: str) -> list[KnowledgeEntry]:
    ...
    async def delete_entry(self, companion_id: str, key: str) -> bool:
    ...
    async def decay_importance(
        ...
        *,
        ...
        if new_importance <= 0:
            await self.delete_entry(companion_id, safe_key)
            return None

        return await self.edit_entry(companion_id, safe_key, importance=new_importance)

    async def _get_entry(self, companion_id: str, safe_key: str) -> KnowledgeEntry | None:
        result = await self._session.execute(
    ...
    def _wiki_dir(self, companion_id: str) -> Path:
        return self._data_dir / companion_id / "wiki"

    def _find_entry_file(self, companion_id: str, safe_key: str) -> Path | None:
        wiki_dir = self._wiki_dir(companion_id)
